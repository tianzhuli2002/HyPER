...
def fill_constituent_row(arr: np.ndarray, event_idx: int, row: np.void) -> None:
    arr[event_idx, 0] = row_to_candidate_features(row)
...
def semantic_ttbar_singlelep_reco(hyper_df: pd.DataFrame) -> pd.DataFrame:
    ...
    if not required_selected.issubset(hyper_df.columns):
        ...
        if "HyPER_CLS_RAW" not in hyper_df.columns:
            hyper_df = hyper_df.copy()
            hyper_df["HyPER_CLS_RAW"] = np.nan
        hyper_df = ttbar_single_lep(hyper_df)
    else:
        hyper_df = hyper_df.copy()
        if "thad_first" not in hyper_df.columns:
            hyper_df["thad_first"] = np.nan
        if "HyPER_CLS_RAW" not in hyper_df.columns:
            hyper_df["HyPER_CLS_RAW"] = np.nan
    ...
    semantic["thad_first"] = hyper_df["thad_first"]
    ...
    semantic["HyPER_CLS_RAW"] = hyper_df["HyPER_CLS_RAW"]
    return semantic
...
def decorate_ttbar_singlelep(
    ...
    jets_ds = h5["INPUTS/JET"]
    leps_ds = h5["INPUTS/LEPTON"]
    met_ds = h5["INPUTS/MET"]
    ...
    for event_idx in range(n_processed):
        source_event_idx = int(start_event) + event_idx
        jet_rows = valid_structured_rows(jets_ds[source_event_idx])
        lepton_rows = valid_structured_rows(leps_ds[source_event_idx])
        met_rows = valid_structured_rows(met_ds[source_event_idx])
        n_jets = len(jet_rows)
        ...
        if whad_valid:
            ...
            fill_constituent_row(outputs["W_HAD_J1"], event_idx, jet_rows[j1])
            fill_constituent_row(outputs["W_HAD_J2"], event_idx, jet_rows[j2])
            whad_p4 = sum(
                (
        ...
        if thad_valid:
            fill_constituent_row(outputs["TOP_HAD_B"], event_idx, jet_rows[b_had_idx])
        ...
        if b_lep_valid:
            fill_constituent_row(outputs["B_LEP"], event_idx, jet_rows[b_lep_idx])
        ...
        if all(math.isfinite(x) for x in (top_had_score, top_lep_score, w_had_score)):
            global_arr["event_reco_score"][event_idx, 0] = (
                top_had_score * top_lep_score * w_had_score
            )
        global_arr["HyPER_CLS_RAW"][event_idx, 0] = finite_float(row["HyPER_CLS_RAW"])
        global_arr["thad_first"][event_idx, 0] = finite_float(row["thad_first"])
...
def decorate_ttH(*_args: Any, **_kwargs: Any) -> None:
    raise NotImplementedError("ttH decoration is intentionally left for a later pass.")
    ...
    with h5py.File(args.output_h5, "a") as output:
        ...
        if args.topology == "ttbar_singlelep":
            ...
            for start in range(0, n_processed, int(args.chunk_size)):
                ...
                LOGGER.info("Decorated events %d:%d", start, stop)
        else:
