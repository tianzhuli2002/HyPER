...
TOPOLOGY_NAME="$(basename "$(dirname "$OUTPUT")")"
RUN_NAME="$(basename "$OUTPUT")"
CENTRAL_LOG_PARENT="$REPO/logs/representation_transfer/$TOPOLOGY_NAME"

mkdir -p "$CENTRAL_LOG_PARENT"
ln -sfnT "$LOG_DIR" "$CENTRAL_LOG_PARENT/$RUN_NAME"
